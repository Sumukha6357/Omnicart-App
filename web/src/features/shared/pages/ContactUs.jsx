    ...
    <div className="mx-auto max-w-4xl space-y-6">
      ...
      <div className="grid gap-6 md:grid-cols-2">
        <div className="marketplace-panel p-5">
          ...
          <p className="mt-3 text-sm text-slate-700">Email: support@omnicart.example</p>
          <p className="mt-1 text-sm text-slate-700">Phone: +1 (555) 123-4567</p>
          <p className="mt-1 text-sm text-slate-700">Hours: Mon-Fri, 9:00am-6:00pm</p>
        </div>

        <form onSubmit={handleSubmit} className="marketplace-panel p-5">
          <h2 className="text-lg font-bold text-slate-900">Get Marketplace Updates</h2>
          <p className="mt-2 text-sm text-slate-600">Subscribe for launch announcements and offer drops.</p>
          <input
            type="email"
            value={email}
            onChange={(e) => setEmail(e.target.value)}
            placeholder="you@example.com"
            className="mt-4 w-full rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm text-slate-800 outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-200"
          />
          <button
            type="submit"
            disabled={loading}
