      ...
      {loading ? (
        ...
        <div className="flex flex-col gap-8">
          {orders.map((order) => {
            ...
            return (
              <div key={order.orderId} className="bg-white shadow rounded p-4">
                        ...
                        return (
                          <tr key={item.productId}>
                            <td className="px-4 py-2 border-b">
                              <img src={product && product.imageUrl ? product.imageUrl : "/placeholder.jpg"} alt={product && product.name ? product.name : "Product"} className="w-12 h-12 object-contain border rounded" />
                            </td>
                            <td className="px-4 py-2 border-b">{product && product.name ? product.name : item.productId}</td>
                            <td className="px-4 py-2 border-b">₹{item.price}</td>
                            <td className="px-4 py-2 border-b">{item.quantity}</td>
                            <td className="px-4 py-2 border-b">₹{item.price * item.quantity}</td>
                          </tr>
                        )
                      })}
                    </tbody>
